...
def setup_feedback_commands(bot):
    ...
    async def feedback_setup_slash(interaction: discord.Interaction):
        """Создает закрепленное сообщение для отзывов"""
    ...
    async def feedback_cleanup_slash(interaction: discord.Interaction):
        """Удаляет старые закрепленные сообщения"""
        allowed_role_ids = [1436746949582786581, 1436748986265374873, 1436746590911074520, 1436748150688714905, 1436747956186386542]
        user_role_ids = [role.id for role in interaction.user.roles]
        has_allowed_role = any(role_id in user_role_ids for role_id in allowed_role_ids)
        ...
        try:
            ...
            for pin in pins:
                if pin.author == bot.user:
                    if pin.components and any(
                        hasattr(btn, 'label') and btn.label == 'Оставить отзыв' 
                        for btn in pin.components[0].children
                    ):
                        await pin.unpin()
                        await pin.delete()
                        deleted_count += 1
            ...
            await interaction.response.send_message(
                f'✅ Удалено {deleted_count} старых закрепленных сообщений!', 
                ephemeral=True, 
                delete_after=10
            )
    ...
    async def feedback_reset_slash(interaction: discord.Interaction):
        """Очищает все отзывы из базы данных"""
        allowed_role_ids = [1436746949582786581, 1436748986265374873, 1436746590911074520, 1436748150688714905, 1436747956186386542]
        user_role_ids = [role.id for role in interaction.user.roles]
        has_allowed_role = any(role_id in user_role_ids for role_id in allowed_role_ids)
        ...
        try:
            ...
            await interaction.response.send_message(
                f'✅ Удалено {deleted_count} отзывов из базы данных!', 
                ephemeral=True, 
                delete_after=10
            )
        except Exception as e:
    ...
    async def feedback_help_slash(interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="📋 Доступные команды системы отзывов",
            description="Список команд для управления отзывами",
            color=0x3498db
        )
        ...
        # Команды управления
        embed.add_field(
            name="⚙️ Управление базой",
            value="`/feedback_reset` - Очистить ВСЕ отзывы из базы",
            inline=False
        )
